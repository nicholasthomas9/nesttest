    ...
    async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<any>> {
        ...
        return next.handle().pipe(
            map(async (data) => {
                ...
                const requestMetricsData = await this.buildMetricsData(request, response, startTime.getTime());
                console.log(requestMetricsData);

                this.metrics.setMetricsData(requestMetricsData);
                await this.metrics.sendMetricsToSqs();
            }), catchError(async (err) => {
                const requestMetricsData = this.buildMetricsData(request, err.response, startTime.getTime());
